...
func TestCustomerDbRepository_DeactivateById(t *testing.T) {
	...
	actualCustomer, statusCode, err := customerDbRepository.DeactivateById(ctx, entity.CustomerIdRequest{
		Id: 1,
	})
	assert.NoError(t, err)
	...
	assert.Equal(t, false, actualCustomer.IsActive)
}
